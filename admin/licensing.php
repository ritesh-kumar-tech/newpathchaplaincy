...
?>
<?php page_header('Licensing', 'Requests', 'Review licensing submissions, affiliations, status, and internal notes.', $total . ' records found', '<a class="btn secondary" href="licensing.php?export=csv">Export CSV</a>'); ?>
<?php if (!empty($_GET['updated'])): ?><p class="form-note success">Licensing request updated.</p><?php endif; ?>
<section class="panel"><form class="toolbar" method="get"><label><span class="sr-only">Search name or email</span><input name="q" value="<?php echo h($search); ?>" placeholder="Search name or email"></label><label><span class="sr-only">Status</span><select name="status"><option value="">All statuses</option><?php foreach ($statuses as $s): ?><option <?php echo $status===$s?'selected':''; ?>><?php echo h($s); ?></option><?php endforeach; ?></select></label><button class="btn primary">Filter</button><a class="btn ghost" href="licensing.php">Reset</a></form><div class="admin-table-wrap"><table class="admin-table review-table"><thead><tr><th>Date</th><th>Applicant</th><th>Affiliation</th><th>Type</th><th>Status</th><th>Review</th></tr></thead><tbody><?php if (!$rows): ?><tr><td colspan="6"><div class="empty-state compact">No licensing requests found.</div></td></tr><?php endif; foreach ($rows as $row): ?><tr><td data-label="Date"><?php echo h(date('M j, Y g:i A', strtotime((string)$row['created_at']))); ?></td><td data-label="Applicant"><strong><?php echo h($row['full_name']); ?></strong><br><a href="mailto:<?php echo h($row['email']); ?>"><?php echo h($row['email']); ?></a><div class="note-box"><?php echo h($row['experience']); ?></div></td><td data-label="Affiliation"><?php echo h($row['affiliation']); ?></td><td data-label="Type"><?php echo h($row['license_type']); ?></td><td data-label="Status"><?php echo status_badge($row['status']); ?></td><td data-label="Review"><form method="post" class="admin-form review-panel" data-change-submit><input type="hidden" name="csrf_token" value="<?php echo h(csrf_token()); ?>"><input type="hidden" name="id" value="<?php echo h($row['id']); ?>"><label>Status<select name="status" aria-label="Licensing status"><?php foreach ($statuses as $s): ?><option <?php echo $row['status']===$s?'selected':''; ?>><?php echo h($s); ?></option><?php endforeach; ?></select></label><label>Internal note<textarea name="internal_notes" placeholder="Internal note"><?php echo h($row['internal_notes']); ?></textarea></label><button class="btn secondary" data-loading-text="Saving..." disabled>Save</button></form></td></tr><?php endforeach; ?></tbody></table></div><?php echo pagination_links('licensing.php', $page, $pages, ['q'=>$search,'status'=>$status]); ?></section>
