...
$search = trim($_GET['q'] ?? ''); $status = trim($_GET['status'] ?? ''); $where=[]; $params=[];
...
$stmt=db()->prepare('SELECT * FROM licensing_requests'.($where?' WHERE '.implode(' AND ',$where):'').' ORDER BY created_at DESC'); $stmt->execute($params); $rows=$stmt->fetchAll();
