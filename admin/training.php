...
<section class="panel"><form method="post" class="admin-form"><input type="hidden" name="csrf_token" value="<?php echo h(csrf_token()); ?>"><?php foreach ($modules as $module): ?><label>Step <?php echo h($module['step_number']); ?> Title<input name="modules[<?php echo h($module['id']); ?>][title]" value="<?php echo h($module['title']); ?>" required></label><label>Step <?php echo h($module['step_number']); ?> Description<textarea name="modules[<?php echo h($module['id']); ?>][description]" required><?php echo h($module['description']); ?></textarea></label><?php endforeach; ?><button class="btn primary">Save Training Cards</button></form></section>
