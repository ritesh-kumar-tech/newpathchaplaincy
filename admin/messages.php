...
if($q!==''){ $where='WHERE full_name LIKE ? OR email LIKE ? OR subject LIKE ?'; $params=["%{$q}%","%{$q}%","%{$q}%"]; }
$stmt=db()->prepare("SELECT * FROM contact_messages {$where} ORDER BY is_read ASC, created_at DESC"); $stmt->execute($params); $rows=$stmt->fetchAll();
